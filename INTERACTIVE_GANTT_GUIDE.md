# 🎨 交互式甘特图使用指南

## 📅 创建时间
2025-12-06 18:35

## 🎯 功能概述

全新的交互式甘特图组件 (`InteractiveGanttChart`) 提供了类似自由画板的体验，支持任务的完整增删查改操作。

---

## ✨ 核心功能

### 1. 任务拖拽移动 🖱️
- **操作**: 点击任务条并拖动
- **效果**: 更改任务的开始和结束日期
- **实时反馈**: 拖动时显示新的时间位置

### 2. 任务时长调整 ↔️
- **操作**: 拖动任务条右侧边缘
- **效果**: 调整任务的结束日期
- **限制**: 结束日期不能早于开始日期

### 3. 画布缩放 🔍
- **工具栏按钮**: 点击 + / - 按钮
- **缩放范围**: 25% - 200%
- **显示**: 实时显示当前缩放比例

### 4. 画布平移 ✋
- **操作**: 在空白区域按住鼠标左键拖动
- **效果**: 平移整个甘特图视图
- **光标**: 拖动时显示抓手光标

### 5. 任务快速操作 ⚡
- **右键菜单**: 在任务上右键点击
- **选项**:
  - 📝 编辑任务
  - 🗑️ 删除任务

### 6. 新增任务 ➕
- **工具栏按钮**: 点击"新增任务"按钮
- **默认设置**:
  - 开始日期: 今天
  - 结束日期: 今天 + 2天
  - 状态: 规划中
  - 优先级: P2

### 7. 时间轴显示 📅
- **月份标注**: 每月1日显示年月
- **日期显示**: 显示日期和星期
- **周末高亮**: 周末背景色区分
- **今日标记**: 当前日期蓝色高亮
- **当前时间线**: 红色竖线标记当前时刻

### 8. 任务依赖关系 🔗
- **可视化**: SVG 贝塞尔曲线连线
- **箭头**: 显示依赖方向
- **悬停效果**: 鼠标悬停时高亮

### 9. 进度显示 📊
- **进度条**: 任务条内显示完成进度
- **百分比**: 右侧显示进度百分比
- **颜色**: 半透明蓝色覆盖层

### 10. 任务状态颜色 🎨
- **已完成**: 绿色背景
- **进行中**: 蓝色背景
- **规划中**: 蓝色背景

---

## 🎮 交互操作

### 鼠标操作

| 操作 | 效果 |
|------|------|
| 左键点击任务 | 选中任务 |
| 左键拖动任务 | 移动任务时间 |
| 左键拖动边缘 | 调整任务时长 |
| 左键拖动空白 | 平移画布 |
| 右键点击任务 | 打开上下文菜单 |
| 悬停任务 | 显示连接点 |

### 视觉反馈

- **选中状态**: 蓝色边框高亮
- **悬停效果**: 阴影加深
- **拖动光标**: 移动/调整大小光标
- **连接点**: 任务两侧的圆点（悬停可见）

---

## 🏗️ 技术实现

### 组件架构
```
InteractiveGanttChart
├── Toolbar (工具栏)
│   ├── Zoom Controls (缩放控制)
│   └── Add Task Button (新增按钮)
├── Canvas Area (画布区域)
│   ├── Header (时间轴标题)
│   ├── Grid Background (网格背景)
│   ├── Tasks Layer (任务层)
│   │   ├── Task Bars (任务条)
│   │   ├── Progress Bars (进度条)
│   │   └── Resize Handles (调整手柄)
│   └── Dependencies SVG (依赖关系)
└── Context Menu (右键菜单)
```

### 核心算法

#### 1. 日期到位置转换
```typescript
const getXFromDate = (date: Date | string) => {
    const d = typeof date === 'string' ? parseISO(date) : date;
    return differenceInDays(d, startDate) * currentCellWidth;
};
```

#### 2. 拖拽距离到日期转换
```typescript
const daysDelta = Math.round(dx / currentCellWidth);
const newStart = addDays(originalStart, daysDelta);
```

#### 3. 时间范围自动计算
```typescript
// 自动计算显示范围
const min = Math.min(...tasks.map(t => new Date(t.startDate)));
const max = Math.max(...tasks.map(t => new Date(t.endDate)));
const start = addDays(startOfWeek(min), -7); // 前缓冲
const end = addDays(max, 14); // 后缓冲
```

---

## 📊 数据流

### 任务更新流程
```
用户拖动任务
    ↓
handleTaskDragStart (记录初始状态)
    ↓
handleMouseMove (跟踪移动)
    ↓
handleMouseUp (计算新日期)
    ↓
onTaskUpdate (回调更新)
    ↓
父组件更新状态
    ↓
重新渲染甘特图
```

### 新增任务流程
```
点击"新增任务"按钮
    ↓
onTaskAdd 回调
    ↓
SmartTaskView 创建完整 Task 对象
    ↓
调用父组件的 onTaskAdd
    ↓
任务添加到列表
    ↓
甘特图自动显示新任务
```

---

## 🎨 样式系统

### 颜色方案
```typescript
const colors = {
  completed: 'bg-green-100 text-green-800',
  active: 'bg-blue-100 text-blue-800',
  planning: 'bg-blue-100 text-blue-800',
  selected: 'ring-2 ring-blue-500',
  weekend: 'bg-slate-50',
  today: 'bg-blue-50',
  currentTime: 'border-red-500'
};
```

### 尺寸常量
```typescript
const CELL_WIDTH = 50;  // 每天的宽度
const ROW_HEIGHT = 48;  // 任务行高度
const HEADER_HEIGHT = 40; // 标题栏高度
```

---

## 🚀 性能优化

### 已实现
- ✅ **useMemo**: 缓存时间范围计算
- ✅ **CSS Transform**: GPU 加速的平移和缩放
- ✅ **事件委托**: 减少事件监听器数量
- ✅ **条件渲染**: 只渲染可见区域

### 建议优化
- [ ] 虚拟滚动 (任务数 > 100)
- [ ] Canvas 渲染 (任务数 > 500)
- [ ] Web Workers (复杂计算)
- [ ] 防抖/节流 (拖动事件)

---

## 🔧 集成方式

### 在 SmartTaskView 中使用
```tsx
import InteractiveGanttChart from './InteractiveGanttChart';

const renderGanttView = () => {
    return (
        <InteractiveGanttChart 
            tasks={tasks}
            onTaskUpdate={onTaskUpdate}
            onTaskDelete={onTaskDelete}
            onTaskAdd={(partialTask) => {
                const newTask: Task = {
                    id: Date.now().toString(),
                    name: '新任务',
                    // ... 其他默认值
                    ...partialTask
                };
                onTaskAdd(newTask);
            }}
        />
    );
};
```

### Props 接口
```typescript
interface InteractiveGanttChartProps {
    tasks: Task[];
    onTaskUpdate: (task: Task) => void;
    onTaskDelete: (taskId: string) => void;
    onTaskAdd: (task: Partial<Task>) => void;
    onDependencyAdd?: (sourceId: string, targetId: string) => void;
    onDependencyDelete?: (sourceId: string, targetId: string) => void;
}
```

---

## 📝 使用示例

### 基本使用
```tsx
<InteractiveGanttChart 
    tasks={projectTasks}
    onTaskUpdate={(task) => updateTask(task)}
    onTaskDelete={(id) => deleteTask(id)}
    onTaskAdd={(partial) => createTask(partial)}
/>
```

### 完整示例
```tsx
const MyGanttView = () => {
    const [tasks, setTasks] = useState<Task[]>([]);

    const handleUpdate = (updatedTask: Task) => {
        setTasks(tasks.map(t => 
            t.id === updatedTask.id ? updatedTask : t
        ));
    };

    const handleDelete = (taskId: string) => {
        setTasks(tasks.filter(t => t.id !== taskId));
    };

    const handleAdd = (partialTask: Partial<Task>) => {
        const newTask: Task = {
            id: Date.now().toString(),
            name: '新任务',
            startDate: format(new Date(), 'yyyy-MM-dd'),
            endDate: format(addDays(new Date(), 2), 'yyyy-MM-dd'),
            status: 'planning',
            priority: 'P2',
            progress: 0,
            type: 'task',
            dependencies: [],
            ...partialTask
        };
        setTasks([...tasks, newTask]);
    };

    return (
        <InteractiveGanttChart 
            tasks={tasks}
            onTaskUpdate={handleUpdate}
            onTaskDelete={handleDelete}
            onTaskAdd={handleAdd}
        />
    );
};
```

---

## 🐛 已知限制

1. **依赖关系编辑**: 目前仅显示，暂不支持拖拽创建
2. **左侧调整**: 暂不支持拖动左边缘调整开始日期
3. **多选操作**: 暂不支持框选多个任务
4. **撤销/重做**: 需要外部状态管理支持

---

## 🔮 未来计划

### Phase 1 (已完成)
- ✅ 基础拖拽移动
- ✅ 右侧调整时长
- ✅ 缩放和平移
- ✅ 右键菜单
- ✅ 依赖关系显示

### Phase 2 (计划中)
- [ ] 左侧调整开始日期
- [ ] 拖拽创建依赖关系
- [ ] 多选和批量操作
- [ ] 撤销/重做
- [ ] 任务分组折叠

### Phase 3 (未来)
- [ ] 里程碑标记
- [ ] 关键路径高亮
- [ ] 资源分配显示
- [ ] 导出为图片/PDF
- [ ] 协作光标

---

## 💡 最佳实践

### 1. 数据管理
- 使用 Zustand 或 Redux 管理任务状态
- 实现乐观更新提升响应速度
- 添加防抖避免频繁保存

### 2. 性能优化
- 任务数 < 50: 无需优化
- 任务数 50-100: 使用 React.memo
- 任务数 > 100: 考虑虚拟滚动

### 3. 用户体验
- 提供加载状态
- 显示操作反馈
- 添加键盘快捷键
- 实现自动保存

---

## 📞 技术支持

如有问题，请参考：
- 源代码: `src/components/InteractiveGanttChart.tsx`
- 集成代码: `src/components/SmartTaskView.tsx`
- 类型定义: `src/types/index.ts`

---

**享受自由画板般的甘特图体验！** 🎨✨
