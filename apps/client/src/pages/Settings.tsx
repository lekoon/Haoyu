...
const Settings: React.FC = () => {
    ...
    // Backup Data
    const handleBackup = () => {
        const data = localStorage.getItem('haoyu-storage');
        if (!data) {
        ...
        a.href = url;
        a.download = `haoyu-backup-${format(new Date(), 'yyyy-MM-dd-HHmmss')}.json`;
        a.click();
        ...
        reader.onload = (e) => {
            try {
                ...
                if (backup.data && backup.version) {
                    localStorage.setItem('haoyu-storage', JSON.stringify(backup.data));
    ...
    const handleClearData = () => {
        if (showClearConfirm) {
            localStorage.removeItem('haoyu-storage');
    ...
    const dataStats = {
        ...
        storageSize: new Blob([localStorage.getItem('haoyu-storage') || '']).size
    };
        ...
        <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500 pb-12">
            ...
            <div className="bg-white dark:bg-slate-800 p-8 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700">
                <div className="mb-6">
                    ...
                    <p className="text-slate-500 dark:text-slate-400">Connect Haoyu with external platforms to sync data.</p>
                </div>
